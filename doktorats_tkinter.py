...
def get_person_registr(person_id):
...
def find_person_window():
    def submit():
        ...
        if person:
            ...
            person_classes = get_person_registr(person_id)
...
def register_for_person_window():
    def submit():
        personal_code = personal_code_entry.get()
        person = find_person_by_personal_code(personal_code)
        if person:
            ...
            messagebox.showinfo("Pieraksts", f"Pacients pierakstīts uz apmeklējumu: {class_type} {class_day} {class_time}")
        else:
...
register_button = tk.Button(root, text="Pieraksts uz nodarbību", command=register_for_person_window)
